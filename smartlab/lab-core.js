/* ============================================================
   INSIGHT SMART LAB — core framework
...
window.InsightLab = (function () {
  ...
  // Classical RK4 over a flat state array. f(t, y, out) writes derivatives.
  ...
  // Compact scientific / fixed notation chosen by magnitude.
  function fmt(v, sig) {
    sig = sig || 3;
    ...
    if (a >= 1e5 || a < 1e-3) {
      ...
      return m.toFixed(sig - 1) + '×10' + supStr(e);
    }
  ...
  // Hex -> rgba with alpha, for glow/fill work on canvas.
  ...
  function Camera(opts) {
    ...
    c.update = function () {
      ...
      const d = [cp * ct, cp * st, sp];                    // target -> eye
      c.eye = [c.target[0] + c.dist * d[0], c.target[1] + c.dist * d[1], c.target[2] + c.dist * d[2]];
      const f = [-d[0], -d[1], -d[2]];                     // forward
      // right = normalize(f x zUp)
    ...
    // Returns screen coords + camera-space depth. Caller checks .ok for near-plane.
  ...
  function Plot(ctx, w, h, cfg) {
    const pad = Object.assign({ l: 48, r: 14, t: 12, b: 28 }, cfg.pad);
    const P = {
      ctx: ctx, w: w, h: h, cfg: cfg, pad: pad,
    ...
    // pts: array of [x,y] in data space
  ...
  /* ---------------- canvas surface with DPR + pointer ---------------- */
  function Surface(el, opts) {
    const cv = document.createElement('canvas');
    el.appendChild(cv);
    const ctx = cv.getContext('2d');
    const S = { el: el, cv: cv, ctx: ctx, w: 0, h: 0, dpr: 1 };
    S.resize = function () {
  ...
  function register(def) { sims.push(def); }

  /* ---------------- runtime ---------------- */
  const R = {
    def: null, S: null, playing: true, speed: 1, raf: 0, lastT: 0,
    stage: null, plot: null, nodes: {}, wtIndex: 0, wtShown: false, uiTimer: 0
  };
  ...
  function buildRail() {
    ...
    SUBJECTS.forEach(sub => {
      ...
      list.forEach(s => {
        ...
        b.innerHTML = '<strong>' + s.name + '</strong><em>' + s.chapter + '</em>';
  ...
  function buildControls(host) {
    ...
    if (def.presets && def.presets.length) {
      ...
      def.presets.forEach(p => {
        const b = el('button', 'preset', p.name);
        b.addEventListener('click', () => {
          Object.assign(S.p, p.params);
          if (def.setup) def.setup(S);
          buildControls(host); syncUI(true);
        });
        pr.appendChild(b);
      });
    ...
    (def.controls || []).forEach(group => {
      ...
      group.items.forEach(it => g.appendChild(buildControl(it)));
      host.appendChild(g);
    });
  }
  ...
  function buildControl(it) {
    ...
    const apply = restructure => {
      ...
      if ((it.restructure || restructure) && def.setup) def.setup(S);
      syncUI(true);
    };
    ...
    // range
    const top = el('div', 'ctl-top');
    const lab = el('span', 'ctl-label', it.label);
    const val = el('span', 'ctl-val');
    top.appendChild(lab); top.appendChild(val);
    const inp = document.createElement('input');
    inp.type = 'range'; inp.id = 'c_' + it.key;
    inp.min = it.min; inp.max = it.max; inp.step = it.step != null ? it.step : (it.max - it.min) / 200;
    inp.value = S.p[it.key];
    const show = () => {
      const v = +inp.value;
      val.innerHTML = (it.fmt ? it.fmt(v) : fmt(v, 3)) + (it.unit ? '<small>' + it.unit + '</small>' : '');
    ...
    wrap._sync = () => { inp.value = S.p[it.key]; show(); };
    return wrap;
  }
  ...
  function renderEquation() {
    ...
    if (!host || !def.equation) return;
    host.innerHTML = def.equation(S);
  }

  /* ---------------- walkthrough ---------------- */
  function renderWalkthrough() {
    const host = R.nodes.wt, def = R.def, S = R.S;
    ...
    if (st.ask) {
      const a = el('div', 'wt-ask', '<b>Predict first</b>' + st.ask);
      body.appendChild(a);
      if (R.wtShown) {
        body.appendChild(el('div', 'wt-reveal', st.reveal));
      } else {
        const b = el('button', 'tbtn', 'Reveal answer');
        b.addEventListener('click', () => { R.wtShown = true; renderWalkthrough(); });
        body.appendChild(b);
      }
    }
    ...
    const prev = el('button', 'tbtn', '← Back');
    const next = el('button', 'tbtn primary', i === steps.length - 1 ? 'Finish' : 'Next →');
    prev.disabled = i === 0;
    prev.style.opacity = i === 0 ? .45 : 1;
    prev.addEventListener('click', () => goStep(i - 1));
    next.addEventListener('click', () => goStep(i === steps.length - 1 ? 0 : i + 1));
  ...
  function mount(id) {
    ...
    ['JEE Main', 'JEE Advanced', 'NEET UG'].forEach(x => {
      const t = el('span', 'tag' + (def.exams.indexOf(x) >= 0 ? ' on' : ''), x);
      tags.appendChild(t);
    });
    ...
    const playBtn = el('button', 'tbtn primary', '❚❚  Pause');
    playBtn.addEventListener('click', () => {
      R.playing = !R.playing;
      playBtn.innerHTML = R.playing ? '❚❚  Pause' : '▶  Play';
    });
    const resetBtn = el('button', 'tbtn', '↺  Reset');
    resetBtn.addEventListener('click', () => {
      R.S.t = 0;
      if (def.setup) def.setup(R.S);
      syncUI(true);
    });
    tp.appendChild(playBtn); tp.appendChild(resetBtn);
    if (def.is3D) {
      const vb = el('button', 'tbtn', '⌖  View');
      vb.addEventListener('click', () => { if (R.S.cam) R.S.cam.reset(); });
      tp.appendChild(vb);
    }
    ...
    /* plot */
    if (def.drawPlot) {
      const pp = panel(def.plotTitle || 'Graph');
      const pbox = el('div', 'plot');
      pp.body.appendChild(pbox);
      if (def.legend) {
        const lg = el('div', 'legend');
        def.legend.forEach(l => {
          const s = el('span', 'lg');
    ...
    R.stage = Surface(stageBox, { orbit: !!def.is3D, cam: () => R.S.cam });
    if (def.setup) def.setup(R.S);
    if (def.walkthrough) renderWalkthrough();
    syncUI(true);
  ...
  function loop(now) {
    ...
    const def = R.def, S = R.S;
    if (!def) return;
    let dt = (now - R.lastT) / 1000;
    R.lastT = now;
    dt = clamp(dt, 0, 0.05);

    if (R.playing && def.step) {
      def.step(S, dt * R.speed);
      S.t += dt * R.speed;
    }

    if (R.stage.resize() || true) {
      R.stage.begin();
      const g = {
        ctx: R.stage.ctx, w: R.stage.w, h: R.stage.h,
        theme: theme, alpha: alpha, mix: mix, now: now / 1000
      };
      if (g.w > 1) {
    ...
    if (R.plot && def.drawPlot) {
      R.plot.resize();
      R.plot.begin();
      if (R.plot.w > 1) {
        def.drawPlot(S, {
          ctx: R.plot.ctx, w: R.plot.w, h: R.plot.h,
          Plot: (cfg) => Plot(R.plot.ctx, R.plot.w, R.plot.h, cfg),
          theme: theme, alpha: alpha, mix: mix
        });
      }
    }
  ...
  function boot() {
    ...
    buildRail();
    mount(sims[0].id);
    window.addEventListener('resize', () => { R.stage && R.stage.resize(); R.plot && R.plot.resize(); });
  }

  return {
    register: register, boot: boot,
    rk4: rk4, clamp: clamp, lerp: lerp, TAU: TAU,
    fmt: fmt, fixed: fixed, sup: supStr, E: E,
    Camera: Camera, Plot: Plot, niceTicks: niceTicks,
    alpha: alpha, mix: mix, theme: theme
  };
})();
